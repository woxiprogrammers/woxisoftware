...
if ( class_exists( 'WP_Importer' ) ) {
	...
	class king_Importer extends WP_Importer {
		...
		function import_start( $file ) {
			if ( ! is_file($file) ) {
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'highstand' ) . '</strong><br />';
				echo __( 'The file does not exist, please try again.', 'highstand' ) . '</p>';
				$this->footer();
				die();
			}
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'highstand' ) . '</strong><br />';
				printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'highstand' ),  $file['file'] );
				echo '</p>';
				return false;
			}
			...
			if ( $this->version > $this->max_wxr_version ) {
				...
				printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'highstand' ), esc_html($import_data['version']) );
				echo '</strong></p></div>';
			}
	...
	<?php if ( ! empty( $this->authors ) ) : ?>
		<h3><?php _e( 'Assign Authors', 'highstand' ); ?></h3>
		<p><?php _e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'highstand' ); ?></p>
	<?php if ( $this->allow_create_users() ) : ?>
		<p><?php printf( __( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'highstand' ), esc_html( get_option('default_role') ) ); ?></p>
	<?php endif; ?>
		<ol id="authors">
	...
	<?php if ( $this->allow_fetch_attachments() ) : ?>
		<h3><?php _e( 'Import Attachments', 'highstand' ); ?></h3>
		<p>
			<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
			<label for="import-attachments"><?php _e( 'Download and import file attachments', 'highstand' ); ?></label>
		</p>
	<?php endif; ?>
	
		<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'highstand' ); ?>" /></p>
	</form>
	<?php
		}
		...
		if ( $create_users ) {
			if ( $this->version != '1.0' ) {
				_e( 'or create new user with login name:', 'highstand' );
				$value = '';
			} else {
				_e( 'as a new user:', 'highstand' );
		...
		if ( ! $create_users && $this->version == '1.0' )
			_e( 'assign posts to an existing user:', 'highstand' );
		else
			_e( 'or assign posts to an existing user:', 'highstand' );
		wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => __( '- Select -', 'highstand' ) ) );
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'highstand' ), esc_html($this->authors[$old_login]['author_display_name']) );
	...
	function process_terms() {
			...
			foreach ( $this->posts as $post ) {
				...
				if ( get_post_type( $post_exists ) != $post['post_type'] ) {
					...
					$postdata = array(
						...
						'post_content' => str_replace( '%SITE_URI%', $xuri, $post['post_content'] ),
		...
		if ( ! $menu_slug ) {
			_e( 'Menu item skipped due to missing menu slug', 'highstand' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			printf( __( 'Menu item skipped due to invalid menu slug: %s', 'highstand' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
		...
		function process_attachment( $post, $url ) {
			...
			if ( ! $this->fetch_attachments )
				return new WP_Error( 'attachment_processing_error',
					__( 'Fetching attachments is not enabled', 'highstand' ) );
			...
			if ( $info = wp_check_filetype( $upload['file'] ) )
				...
				return new WP_Error( 'attachment_processing_error', __('Invalid file type', 'highstand') );
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( ! $headers ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Remote server did not respond', 'highstand') );
			}
	
			// make sure the fetch was successful
			if ( $headers['response'] != '200' ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf( __('Remote server returned error response %1$d %2$s', 'highstand'), esc_html($headers['response']), get_status_header_desc($headers['response']) ) );
			}
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Remote file is incorrect size', 'highstand') );
			}
	
			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Zero size file downloaded', 'highstand') );
			}
	
			$max_size = (int) $this->max_attachment_size();
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf(__('Remote file is too large, limit is %s', 'highstand'), size_format($max_size) ) );
			}
		...
		function greet() {
			...
			echo '<p>'.__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'highstand' ).'</p>';
			echo '<p>'.__( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'highstand' ).'</p>';
