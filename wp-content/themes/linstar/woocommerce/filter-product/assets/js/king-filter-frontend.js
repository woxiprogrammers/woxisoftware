...
jQuery(function($){
    ...
    //wrap the container
    $(king_filter.container).wrap('<div class="dev-filter-container"></div>');
    $('.woocommerce-info').wrap('<div class="dev-filter-container"></div>');

   $(document).on('click', '.king-filter a', function(e){
       ...
       if( $(this).data('type') == 'select' ) {
           ...
           $(this).parents('div.king-filter-ajax-navigation').find('a.dev-filter-select-open').removeClass('active');

           $(this).parent().find('div.king-filter-select-wrapper').animate({
        ...
        //loading
        $(king_filter.container).html('').addClass('king-filter-loading');
        $(king_filter.pagination).hide();
        $(king_filter.result_count).hide();

        $.ajax({
            url: href,
            success: function( response ){
                $(king_filter.container).removeClass('king-filter-loading');

                //container
                if( $(response).find(king_filter.container).length > 0 ) {
                    $('.dev-filter-container').html( $(response).find(king_filter.container) );
                } else {
                ...
                //pagination
                if( $(response).find(king_filter.pagination).length > 0 ) {
                    //se non esiste lo creo
                    if( $(king_filter.pagination).length == 0 ) {
                        $.jseldom( king_filter.pagination ).insertAfter( $(king_filter.container) );
                    }

                    $(king_filter.pagination)
                        .html( $(response).find(king_filter.pagination).html())
                        .show();
                }

                //result count
                if( $(response).find(king_filter.result_count).length > 0 ) {
                    $(king_filter.result_count).html( $(response).find(king_filter.result_count).html()).show();
                }


                //load new widgets
                $('.king-filter-ajax-navigation').each(function(){
                    var id = $(this).attr('id');
                ...
                $(document).trigger("king-filter-ajax-filtered");
            }
    ...
    function dev_open_select_dropdown(element){
        ...
        $(element).parent().find('div.king-filter-select-wrapper').css("z-index", "1").animate({
    ...
    var dev_hidden_filters_wrapper = function () {
        ...
        $('div.king-filter-select-wrapper').animate({
    ...
    var dev_active_filter = function() {
        ...
        var filter_number = $('div.king-filter-select-wrapper ul.king-filter-select li.chosen').length;

        dev_hidden_filters_wrapper();

        $('div.king-filter-select-wrapper').each(function() {

            var filter_name="";
            var chosen =  $(this).find('ul.king-filter-select li.chosen').each(function(){
    ...
    $(document).on('ready king-filter-ajax-filtered', dev_active_filter);
